...
The following wraps up how to build `IntaRNA-2.3.0` under OS X (Mojave 10.14.1) using homebrew.
...
IntaRNA requires ViennaRNA version 2.4.8+. In order to install it, create a
file named ViennaRNA.rb with the following content:
...
class Viennarna < Formula
  ...
  url "https://www.tbi.univie.ac.at/RNA/packages/source/ViennaRNA-2.4.8.tar.gz"
  sha256 "1f8a08b1c8e57e4c6610e0e48c66ae692caba4183867cde8eb58ec16c40f78fa"
...
Download and extract the IntaRNA source code package (e.g. `intaRNA-2.3.0.tar.gz`) from the [release page](releases/).
...
subsequences are probably involved in intramolecular base pairings. Furthermore,
...
- `--windowOverlap` : overlap of the windows that has to cover the maximal interaction length (see [`--q|tIntLenMax`](#interConstr))   
...
The most general restriction is the maximal energy (inversely related to
