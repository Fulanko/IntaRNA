...
AccessibilityBasePair::AccessibilityBasePair(const RnaSequence& seq,
    ...
    const E_type bpEnergy, const Z_type _RT, const size_t minLoopLen) :
      ...
      RT(_RT),
      basePairWeight( _RT == 0.0 ? 0.0 : boost::multiprecision::exp(E_2_Z(-bpEnergy) / _RT) ),
      minLoopLength(minLoopLen)
{
  ...
  for (size_t i = 0u; i < N; ++i) {
    for (size_t j = i; j < N; ++j) {
      logPu(i, j) = Z_2_E( -RT * boost::multiprecision::log(Z_type(NussinovHandler::getPu(i, j, seq, basePairWeight, minLoopLength, Q, Qb, Pbp, Pu))) );
    }
  }
}
