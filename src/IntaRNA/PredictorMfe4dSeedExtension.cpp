...
PredictorMfe4dSeedExtension::
PredictorMfe4dSeedExtension( const InteractionEnergy & energy
		...
		)
{
	...
	if (!(r1.isAscending() && r2.isAscending()) )
		throw std::runtime_error("PredictorMfe4dSeedExtension::predict("+toString(r1)+","+toString(r2)+") is not sane");
#endif
...
traceBack( Interaction & interaction, const OutputConstraint & outConstraint  )
{
	...
	// sanity checks
	if ( interaction.basePairs.size() != 2 ) {
		throw std::runtime_error("PredictorMfe4dSeedExtension::traceBack() : given interaction does not contain boundaries only");
	}
#endif
	...
	if ( ! interaction.isValid() ) {
		throw std::runtime_error("PredictorMfe4dSeedExtension::traceBack() : given interaction not valid");
	}
#endif
