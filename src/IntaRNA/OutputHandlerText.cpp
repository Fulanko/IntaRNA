...

	{
		...
		if (detailedOutput) {
			outTmp
				...
				<<"    : Pu(seq1)     = "<<E_2_Ekcal(Z_2_E(boost::multiprecision::exp(-E_2_Z(contr.ED1)/energy.getRT())))<<'\n'
				<<"  + ED(seq2)       = "<<E_2_Ekcal(contr.ED2)<<'\n'
				<<"    : Pu(seq2)     = "<<E_2_Ekcal(Z_2_E(boost::multiprecision::exp(-E_2_Z(contr.ED2)/energy.getRT())))<<'\n'
				;
