...
add( const Interaction & i )
{
	...
	// ensure outputs do not intervene
	{
		...
		for (auto col = colOrder.begin(); col != colOrder.end(); col++) {
			...
			case Pu1:
			  outTmp <<std::exp( E_2_Ekcal( - contr.ED1 ) / energy.getRT() );
				break;

			case Pu2:
				outTmp <<std::exp( E_2_Ekcal( - contr.ED2 ) / energy.getRT() );
				break;
			...
			case seedPu1:
				if (i.seed == NULL) {
					...
					outTmp <<std::exp( E_2_Ekcal( - energy.getED1( i.seed->bp_i.first, i.seed->bp_j.first ) ) / energy.getRT() );
				}
				break;
			...
			case seedPu2:
				if (i.seed == NULL) {
					...
					outTmp <<std::exp( E_2_Ekcal( - energy.getAccessibility2().getAccessibilityOrigin().getED( i.seed->bp_j.second, i.seed->bp_i.second ) ) / energy.getRT() );
				}
				break;
